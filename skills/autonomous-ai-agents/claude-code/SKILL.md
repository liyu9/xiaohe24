...
`--bare` skips hooks, plugins, MCP discovery, and CLAUDE.md loading. Fastest startup.

**Caveat (v2.1+):** `--bare` no longer fully bypasses login. As of v2.x, Claude Code still prompts `/login` even with `--bare` if it doesn't see a valid auth state. The `--bare` flag's "strictly ANTHROPIC_API_KEY" promise only applies when you also use it together with the 3P-provider env-var pattern below. For unattended CI: set BOTH `ANTHROPIC_API_KEY` and use a 3P-provider base URL (see "3P Providers" below). For local interactive use: run `claude auth login --console` once and let the keychain store the token.

### 3P Providers (minimax, OpenRouter, etc.) — Working Pattern

Claude Code v2.x supports first-class 3P-provider routing via env vars. **No `--login`, no `claude-code-router`, no `ccr` needed.** Set these:

```
export ANTHROPIC_BASE_URL="https://api.minimaxi.com/anthropic"   # provider's Anthropic-compatible endpoint
export ANTHROPIC_AUTH_TOKEN="<provider-api-key>"                 # NOT ANTHROPIC_API_KEY
export ANTHROPIC_MODEL="MiniMax-M3"                              # or whatever model the provider exposes
...
**Why these specific env var names:**
- `ANTHROPIC_BASE_URL` (not `OPENAI_BASE_URL`) — Claude Code speaks the Anthropic protocol regardless of provider
- `ANTHROPIC_AUTH_TOKEN` (not `ANTHROPIC_API_KEY`) — the v2.x custom-auth path
- The three `ANTHROPIC_DEFAULT_*_MODEL` vars pin the three built-in aliases (sonnet/opus/haiku) to your provider's model. Without these, Claude Code tries to call e.g. `claude-3-5-sonnet-*` and the provider 404s.

**Verify the base URL by testing directly first** (curl + the same auth header pattern the provider expects) — if your provider's `/v1/messages` endpoint works with `x-api-key: <token>` + `anthropic-version: 2023-06-01`, the env-var pattern above will work end-to-end.

**Permission gotcha for print mode:** Claude Code's Write/Edit/Bash tools require explicit `--allowedTools` allowlisting in print mode, otherwise the first file-write attempt gets a `permission_denials` entry in the JSON output and the task fails. Pattern:

```
claude -p 'Create foo.py with X' --max-turns 5 --allowedTools 'Write,Bash,Edit,Read'
```

**Cost/latency note:** Set `ANTHROPIC_SMALL_FAST_MODEL` to the provider's haiku-equivalent if available — Claude Code uses it internally for some sub-tasks, and 3P providers often have a faster/cheaper small model.

**Wrapper script pattern** (recommended for Hermes to call into Claude Code without leaking env vars to subshells):

```bash
#!/usr/bin/env bash
# ~/.hermes/bin/claude_code
set -euo pipefail
export ANTHROPIC_BASE_URL="https://..."
export ANTHROPIC_AUTH_TOKEN="$(grep ^ANTHROPIC_AUTH_TOKEN= ~/.hermes/.env | cut -d= -f2)"
export ANTHROPIC_MODEL="..."
# ... all the above
exec /path/to/claude "$@"
```

Keeps the key in `~/.hermes/.env` (chmod 600), not in `~/.bashrc` or per-call args.
...
11. **`--bare` does NOT fully skip login in v2.1+** — Claude Code still prompts `/login` even with `--bare` if no valid auth state. For unattended/3P use, set the full 3P-provider env-var set (see "3P Providers" section), not just `ANTHROPIC_API_KEY`.
12. **Context degradation is real** — AI output quality measurably degrades above 70% context window usage. Monitor with `/context` and proactively `/compact`.

## Reference files

- `templates/claude_code_progress_card.py` — drop-in Python script for streaming a Claude Code task's progress to a Feishu chat as an interactive card. Sends initial card, PATCHes every 200ms with iteration/stage, sends a final result card. CardKit 2.0 schema-verified (correct tag names, throttle, seq for ordering). Use it when a long Claude Code task needs visible progress in chat without flooding the channel.

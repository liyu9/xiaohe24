...
## Pitfalls (verified 2026-06-18 on real WeChat URL)

- **Bundled `extract_wechat.py` can silently degrade on raw WeChat HTML.** In a test session it printed "Could not find main content block, using full HTML" and produced 20 images / 5,676 chars when the correct count is 18 / 5,248. The cause: the script's `find_content_block` uses `html.find(selector)` against a 3MB page and somehow didn't lock onto the right slice. Symptom: extra images and a few hundred chars of nav/header bleed-through. **Fix**: if the script output has noticeably more images than expected (rough sanity check: ~1 image per 250 chars of body), re-run the manual recipe in `references/wechat-html-quirks.md`.
- **WeChat's `js_content` div ships with `style="visibility: hidden; opacity: 0;"`** in the static HTML. The content is fully there — don't reject the page thinking it's empty. The script's `find_content_block` should match `id="js_content"`, but if it falls through, the issue is elsewhere (encoding, slicing, or selector ordering).
- **End-of-article sentinel** on raw WeChat HTML is `<p style="display: none;"><mp-style-type data-value="10000">`. Cut on `mp-style-type` — do not trust the script's `FOOTER_MARKERS` heuristic on raw WeChat (those markers are mirror-site footer text and won't appear in raw `mp.weixin.qq.com` HTML).
- **Title source**: WeChat's `<title>` is empty. Use `og:title` (always present) and `og:description` (often a 1-sentence summary) for header. Author/date are not in standard meta — fall back to "苍何" style JS-rendered footer if `extract_metadata` returns empty author.
- **Entity decode order**: when entities are decoded BEFORE remaining tags are stripped, an `&lt;` from a code sample becomes a raw `<` that the next `<[^>]+>` pass treats as a tag. In the manual recipe we strip tags first, then `html.unescape`. The script's order works for common articles but breaks on code-heavy content.

## When the script fails — recovery protocol

1. **Check input is real WeChat**: `grep -c 'id="js_content"' /tmp/file.html` should be ≥ 1. If 0, you got a CAPTCHA / redirect — go back to Step 2-3.
2. **Check file size**: WeChat articles are 2-4 MB. <500 KB means it's a CAPTCHA or error page.
3. **Sanity-check script output**: count `![` in the markdown. WeChat articles typically have 5-30 inline images. If you see >50, the script fell back to full-page and is including nav/header images.
4. **If output has nav bleed-through**: use the manual recipe in `references/wechat-html-quirks.md`. Saves a 2-3 minute debugging cycle vs fighting the script.
...
- See `references/wechat-html-quirks.md` for the manual extraction recipe and full diagnostic checklist
- Linked support files: `references/wechat-html-quirks.md` (diagnostic + manual recipe); `scripts/extract_wechat.py` (bundled extractor — works for most cases, has known degradation on raw WeChat HTML)
